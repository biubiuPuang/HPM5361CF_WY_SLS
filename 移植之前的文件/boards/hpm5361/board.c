...
#define BOARD_APP_SPI_CLK_NAME       clock_spi0
#define BOARD_APP_ADC_CLK_NAME       clock_adc0
...
#define BOARD_SYSTICK_FREQ (100UL)
...
//      // board_init_usb_pins();
    //board_init_spi_pins_with_gpio_as_cs();
...
{
    bsp_gpio_write_pin(BOARD_LED_GPIO_CTRL, BOARD_LED_GPIO_INDEX, pin, state);
}
...
//     pmp_entry[0].paddr = 0;
//     pmp_entry[0].size = PMP_SIZE_4G;
//     pmp_entry[0].prot.val = PMP_PROT_ANY_EXECUTE | PMP_PROT_READ | PMP_PROT_WRITE;
//     pmp_entry[0].cfg.val = PMP_CFG_NAPOT | PMP_CFG_VALID;
//     pmp_entry[0].lock = true;

//     pmp_config(0, pmp_entry, ARRAY_SIZE(pmp_entry));
// 
}
...
void board_init_key_pins(void)
{
    ...
    // {
    //     board_delay_ms(10);
    //     if (gpio_read_pin(BOARD_KEY1_GPIO, BOARD_KEY1_PORT, BOARD_KEY1_PIN) == 1)
    //     {
    //         break;
    //     }
    // }
